...
function convertWebResetFormat(webFormat) {
  if (!webFormat) return '';
  ...
  const relativeMatch = webFormat.match(/^in\s+(?:(\d+)\s*hr?)?\s*(?:(\d+)\s*min)?/i);
  if (relativeMatch) {
    const hours = relativeMatch[1] ? parseInt(relativeMatch[1]) : 0;
    const minutes = relativeMatch[2] ? parseInt(relativeMatch[2]) : 0;
    const resetDate = new Date(now.getTime() + hours * 3600000 + minutes * 60000);
    ...
    return `${hour}:${minStr}${ampm} (${tz})`;
  }

  // Handle "Mon 12:00 PM" format
  const weekdayMatch = webFormat.match(/^(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+(\d{1,2}):(\d{2})\s*(AM|PM)/i);
  if (weekdayMatch) {
    ...
    let displayHour = resetDate.getHours();
    const displayAmpm = displayHour >= 12 ? 'pm' : 'am';
    displayHour = displayHour % 12 || 12;
    const minStr = resetDate.getMinutes().toString().padStart(2, '0');

    return `${month} ${day}, ${displayHour}:${minStr}${displayAmpm} (${tz})`;
  }
...
function parseUsageText(text) {
  ...
  return {
    sessionUsed,
    sessionResets: convertWebResetFormat(sessionResets),
    weeklyUsed,
    weeklyResets: convertWebResetFormat(weeklyResets),
  };
}
